...
while True:
    ...
    for r in records_1:
        ...
        ma_1 = {'timestamp': int(r['timestamp']), 'amount': float(data_1['value'].tail(n=params['ma']).mean())}
        # produce data
    ...
    for r in records_2:
        ...
        ma_2 = {'timestamp': int(r['timestamp']), 'amount': float(data_2['value'].tail(n=params['ma']).mean())}
        # produce data
    ...
    for r in records_3:
        ...
        ma_3 = {'timestamp': int(r['timestamp']), 'amount': float(data_3['value'].tail(n=params['ma']).mean())}
        # produce data
